...
\documentclass[dvipdfmx,uplatex,papersize,twoside,b5j,10pt,openright]{jsbook}
% %% fixes to LaTeX2e
...
\usepackage[dvipdfmx,bookmarks=true,bookmarksnumbered=true,colorlinks=true,%
     pdftitle={LINE API ExpertによるLINE API本},%
     pdfauthor={LINE Experts}]{hyperref}
...
%%%
\title{LINE API ExpertによるLINE API本}
\makeatletter
\def\@subtitle{}
\def\@pubevent{技術書典7}
\def\@latestpubhistory{2019年9月22日　ver 1.0}
\makeatother
\author{LINE Experts　著}
...
\begin{center}%
  \mbox{} \vskip5zw
   \reviewtitlefont%
    {\HUGE\bfseries LINE API ExpertによるLINE API本 \par}%
    \vskip 1em%
...
\input{chapxx-4geru.tex}
\input{chapxx-gaomar.tex}
\input{chapxx-hirarin.tex}
\input{chapxx-kenakamu.tex}
\input{chapxx-kobayashi-hirotada.tex}
\input{chapxx-miso.tex}
\input{chapxx-mr-mo.tex}
\input{chapxx-n0bisuke.tex}
\input{chapxx-nisshi79.tex}
\input{chapxx-shinbunbun.tex}
\input{chapxx-shoya-shiraki.tex}
\input{chapxx-sumihiro3.tex}
\input{chapxx-wakasa.tex}
...
%
  ...
  \def\colophonrights{%
    ...
    {\textcopyright} 2019 LINE Experts\\
...
{\noindent\reviewtitlefont\Large LINE API ExpertによるLINE API本} \\
\medskip
